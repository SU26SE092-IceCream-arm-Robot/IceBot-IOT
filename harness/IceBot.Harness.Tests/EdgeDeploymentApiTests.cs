...
namespace IceBot.Harness.Tests
{
    public class EdgeDeploymentApiTests
    {
        ...
        [Fact]
        public void BuildAcknowledgementBody_IncludesPhysicalEvidenceOnlyForRejectedOrder()
        {
            var accepted = EdgeDeploymentApi.BuildAcknowledgementBody(
                "Accepted", true, null, null, null);
            var rejectedDeployment = EdgeDeploymentApi.BuildAcknowledgementBody(
                "Rejected", false, "DeploymentInstallRejected", "invalid bundle", null);
            var rejectedOrder = EdgeDeploymentApi.BuildAcknowledgementBody(
                "Rejected", false, "ArtifactMissing", "missing Lua", false);

            Assert.False(accepted.ContainsKey("physicalOutputMayHaveOccurred"));
            Assert.False(rejectedDeployment.ContainsKey("physicalOutputMayHaveOccurred"));
            Assert.Equal(false, rejectedOrder["physicalOutputMayHaveOccurred"]);
        }
    }
}