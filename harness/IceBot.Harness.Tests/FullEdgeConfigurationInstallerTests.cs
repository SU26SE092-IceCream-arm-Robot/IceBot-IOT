...
namespace IceBot.Harness.Tests
{
    public class FullEdgeConfigurationInstallerTests
    {
        ...
        [Fact]
        public void DeploymentReportOutbox_OrdersInstalledBeforeActiveByDurableSequence()
        {
            var directory = TempDirectory();
            try
            {
                File.WriteAllText(Path.Combine(directory, "command-Active.json"),
                    JsonSerializer.Serialize(new DeploymentReportData { Status = "Active", SequenceNumber = 2 }));
                File.WriteAllText(Path.Combine(directory, "command-Installed.json"),
                    JsonSerializer.Serialize(new DeploymentReportData { Status = "Installed", SequenceNumber = 1 }));

                var ordered = DeploymentReportOutbox.ListPendingInDeliveryOrder(directory);

                Assert.Equal(new[] { "command-Installed.json", "command-Active.json" },
