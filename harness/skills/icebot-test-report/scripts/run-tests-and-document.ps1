param([string[]]$TestClasses=@(),[switch]$NoRestore,[ValidateSet('Debug','Release')][string]$Configuration='Release')
...
function Run([string]$name,[string]$filter){$args=@('test',$project,'--configuration',$Configuration,'--logger',"trx;LogFileName=$name",'--results-directory',$results);if($NoRestore){$args+='--no-restore'};if($filter){$args+=@('--filter',$filter)};& dotnet @args;if($LASTEXITCODE-ne 0){throw "Tests failed: $name"}}
